...
export const Banner: React.FC = () => {
  const [BannerUser, setBannerUser] = useState<BannerUserType[]>([]);
  ...
  return (
    <div>
      <PageMeta
        ...
        description="This is the Banner page"
      />
...
interface BannerData {
  heading: string;
  description: string;
  bannerimage1: File | null;
  bannerimage2: File | null;
}
...
export const BannerAdd: React.FC = () => {
  ...
  const imageHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      img.onload = () => {
        ...
        if (fieldName === "bannerimage1") {
          setBannerData((prev) => ({ ...prev, bannerimage1: file }));
          setPreviewImage(objectUrl);
        } else if (fieldName === "bannerimage2") {
          setBannerData((prev) => ({ ...prev, bannerimage2: file }));
  ...
  const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      !bannerData.heading ||
      !editorData ||
      !bannerData.bannerimage1 ||
      !bannerData.bannerimage2
    ) {
...
export const BannerEdit: React.FC = () => {
  const [Banneruser, setBannerUser] = useState<BannerEditData>({
    heading: "",
    heading1: "",
    description: "",
    bannerimage1: "",
  ...
  useEffect(() => {
    if (editorLoaded) fetchBannerData();
  }, [editorLoaded, id]);
  ...
  const changeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setBannerUser({ ...Banneruser, [name]: value });
  };
  ...
  const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const formData = new FormData();
    formData.append("heading", Banneruser.heading);
    formData.append("heading1", Banneruser.heading1);
    formData.append(
      "description",
      (window as any).CKEDITOR.instances.editor.getData()
    );
    if (selectedImage1) formData.append("bannerimage1", selectedImage1);
    if (selectedImage2) formData.append("bannerimage2", selectedImage2);

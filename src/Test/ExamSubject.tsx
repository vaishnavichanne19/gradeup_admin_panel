...
export const ExamSubject: React.FC = () => {
  const [Exams, setExams] = useState<ExamUserType[]>([]);
  ...
  const DeleteSubject = async (userId: string) => {
    try {
      ...
      toast.error("Subject Deleted Sccessfully!");
...
const filteredSubjectData = useMemo(() => {
  return subjects
    .map((subj) => {
      const Exam = Exams.find((b) => b._id === subj.entranceexamref);
      if (!Exam) return null;
  ...
  return (
    <div>
      <PageMeta
        ...
        description="This is the Exam Subject page"
      />
      <PageBreadcrumb pageTitle="Exam Subject" />
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
           Exam Subject Information
          </h3>
          <Link to="/ExamSubject-add">
            ...
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {paginatedSubject.length > 0 ? (
                  paginatedSubject.map((sub, index) => (
                    <TableRow key={sub._id}>
                      ...
                      <TableCell className="px-4 py-3 text-purple-700 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex gap-3">
                         <Link to={`/ExamSubject-edit/${sub._id}`}>
                                <i className="fa-solid fa-pencil align-middle"></i>
                              </Link>
                              <span onClick={() => DeleteSubject(sub._id)}>

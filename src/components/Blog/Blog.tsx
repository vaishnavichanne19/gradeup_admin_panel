...
export const Blog: React.FC = () => {
  const [BlogUser, setBlogUser] = useState<BlogUserType[]>([]);
  ...
  const handleTogglePublish = async (
    ...
    const updatedValue = !currentStatus;
    ...
    try {
      ...
      setBlogUser((prevUsers) =>
        prevUsers.map((user) =>
          user._id === itemId ? { ...user, publish: updatedValue } : user
        )
      );

      if (updatedValue) {
        toast.success("Blog is now published");
      } else {
        toast.warning("Blog has been unpublished");
      }
    } catch (error) {
  ...
  const DeleteData = async (userId: string) => {
    try {
      ...
      toast.error("Data Deleted Sccessfully!");
      navigate("/Blog");
    } catch {
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
          ...
          <div className="flex flex-col gap-6">
            <div className="w-full border p-4 rounded-lg shadow-md dark:border-gray-700">
              <div className="flex items-start justify-between">
                ...
                <Link to={`/Blog-edit/${"686d05474e3da9f36015c5f0"}`}>
        ...
        <div className="mt-7 overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {BlogUser.map((user, index) => (
                  <TableRow key={user._id}>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                      <div className="flex -space-x-2">{user.date}</div>
                    </TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                      <div className="flex -space-x-2">{user.heading}</div>
                    </TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                      <div
                      ...
                      <div className="flex -space-x-2">
                        <div className="form-check form-switch">
                          <input
                            ...
                            checked={!!user.publish}
                            onChange={() =>
                              handleTogglePublish(user._id, !!user.publish)
                            }
                          />

                          <label
                            className={`form-check-label font-semibold ${
                              !!user.publish ? "text-green-600" : "text-red-600"
                            }`}
                            htmlFor={`PublishSwitch-${user._id}`}
                          >
                            {" "}
                            {!!user.publish ? "Publish" : "UnPublish"}
                          </label>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                      <div className="flex -space-x-2">
                        <div className="flex gap-2">
                          <Link
                            to={`/Blog-data-edit/${user._id}`}
                            className="btn btn-sm"
                          >
                            <i className="fa-solid fa-pencil align-middle fs-18"></i>
                          </Link>
                          <span
                            onClick={() => DeleteData(user._id)}
                            className="btn btn-soft-danger btn-sm"
                          >
                            <i className="fa-solid fa-trash align-middle fs-18"></i>
                          </span>
                          {/* <Link to={`/Blog-detail/${user._id}`}>
                            <button className="bg-blue-700 text-white px-2 py-1">View Detail</button>
                          </Link> */}
                        </div>
                      </div>
                    </TableCell>
...
//   const [BlogDetailUser, setBlogDetailUser] = useState<BlogDetailUserType>({
//     heading: "",
//     date: "",
//     blogdetail: "",
//     blogimage: "",
...
//       />
//       <PageBreadcrumb pageTitle="Blog Detail" />
//       <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
//         <div className="mb-4 flex items-center justify-between">
//           <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
...
//                   </p>
//                 </div>
//                 <Link to={`/Blog-detail-edit/${BlogDetailUser._id}`}>
//                   <button className="flex items-center justify-center gap-2 rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
//                     <svg
//                       className="fill-current"
//                       width="18"
...
//                     __html: BlogDetailUser.blogdetail,
//                   }}
//                 ></p>
//               </div>
//             </div>
    ...
    if (file) {
      ...
      img.onload = () => {
        ...
        if (fieldName === "blogimage") {
          setBlogData((prev) => ({ ...prev, blogimage: file }));
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 flex p-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 w-full max-w-xl border border-gray-300">
        ...
        <form onSubmit={submitForm}>
          ...
          <label className="block text-gray-700 dark:text-white text-sm">
            Date
          </label>
          <input
            type="text"
...
export const BlogEdit: React.FC = () => {
  const [Bloguser, setBlogUser] = useState<BlogEditData>({
    heading: "",
    heading1: "",
    description: "",
    blogimage: "",
  ...
  const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const formData = new FormData();
    formData.append("heading", Bloguser.heading);
    formData.append("heading1", Bloguser.heading1);
    formData.append(
      "description",
      (window as any).CKEDITOR.instances.editor.getData()
    );
    if (selectedImage1) formData.append("blogimage", selectedImage1);
...
export const BlogDataEdit: React.FC = () => {
  const [Bloguser, setBlogUser] = useState<BlogEditTableData>({
    heading: "",
    description: "",
    blogimage: "",
    date: "",
  ...
  const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("date", Bloguser.date);
    if (selectedImage1) formData.append("blogimage", selectedImage1);
...
interface BlogDetailEditTableData {
  blogdetail: string;
  heading: string;
  blogimage: string;
  date: string;
...
export const BlogDetailEdit: React.FC = () => {
  const [Bloguser, setBlogUser] = useState<BlogDetailEditTableData>({
    heading: "",
    blogdetail: "",
    blogimage: "",
    date: "",
  ...
  const fetchBlogData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const interval = setInterval(() => {
          if ((window as any).CKEDITOR?.instances?.editor) {
            (window as any).CKEDITOR.instances.editor.setData(data.blogdetail);
            clearInterval(interval);
          }
        }, 100);
      }
